...
vehicleStore.getVehicles().then((response) => {
  if (response.length > 0) {
    parkingStore.form.vehicleId = response[0].id;
  }
});
 
zoneStore.getZones().then((response) => {
  if (response.length > 0) {
    parkingStore.form.zoneId = response[0].id;
  }
});
  ...
  <form @submit.prevent="parkingStore.startParking" novalidate>
    <div class="flex flex-col mx-auto md:w-96 w-full">
      ...
      <div class="flex flex-col gap-2 mb-4">
        ...
        <select
          v-model="parkingStore.form.vehicleId"
          name="vehicle_id"
          id="vehicle_id"
          class="form-input"
        ...
        <ValidationError :errors="parkingStore.errors" field="vehicleId" />
      </div>
 
      <div class="flex flex-col gap-2">
        <label for="zone_id" class="required">Zone</label>
        <select
          v-model="parkingStore.form.zoneId"
          name="zone_id"
          id="zone_id"
          class="form-input"
          :disabled="parkingStore.loading"
        ...
        <ValidationError :errors="parkingStore.errors" field="zoneId" />
